import type { INodeWorker, TMode } from "./node.types";
...
export class RenderNode {
	...
	private _updateLocationNodes_(nodes: INodeWorker[], parent?: INodeWorker) {
	...
	private _traverse_(
		callback: (node: INodeWorker | INodeWorker) => void,
		node: INodeWorker | INodeWorker,
	) {
		callback(node);
	...
	static calculateTransforms(
		...
		const globalRotation = nodeTransform.rotation + parentTransform.rotation;
	...
	traverse(callback: (node: INodeWorker | INodeWorker) => void) {
		this._traverse_(callback, this._root_[0]);
	}
	...
	getNodeByPath(path: string, mode: TMode = "index"): INodeWorker | undefined {
	...
	protected getNodeByPathId(path: string): INodeWorker | undefined {
	...
	protected getNodeByPathSlug(path: string): INodeWorker | undefined {
		const parts = path.split("/");
	...
	protected getNodeByPathIndex(path: string): INodeWorker | undefined {
		return this._nodes_(`[${path.replace(/\//g, "].nodes[")}]`)(this._root_);
	}
	...
	protected getNodeById(location: string): INodeWorker | undefined {
		return this._root_.find((node) => node.location.id === location);
	}

	protected getNodeBySlug(location: string): INodeWorker | undefined {
		return this._root_.find((node) => node.location.slug === location);
	}

	protected getNodeByIndex(location: number): INodeWorker | undefined {
		let left = 0;
	...
	protected getParentNodeByPathId(path: string) {
		...
		let parentPath =
			$node.__path__.indexOf("/") !== -1
				? $node.__path__.slice(0, -2)
				: $node.__path__;

		parentPath = `[${parentPath.replace(/\//g, "].nodes[")}]`;

		const $parent: INodeWorker | undefined = this._nodes_(parentPath)(
			this._root_,
		);
