export class ControllerAnimationFrame {
  ...
  protected _delayWithFramePerSeconds =
    this._delayTime / this._velocityFramesPerSeconds
  ...
  protected calculateFPS() {
    ...
    if (delayTime > this._delayTime) {
      this._state.fps = Math.round(
        (this._state.frames * this._delayTime) / delayTime
      )
  ...
  protected loop(timestamp: number, self: any) {
    ...
    let seconds = Math.floor(
      (timestamp - self._time) / self._delayWithFramePerSeconds
    )
    ...
    self._animationFrameRef = window.requestAnimationFrame((ts) =>
      self.loop(ts, self)
    )
  }
  ...
  public play() {
    if (!this._status.playing) {
      ...
      this._animationFrameRef = window.requestAnimationFrame((ts) =>
        this.loop(ts, this)
      )
    }
  }
