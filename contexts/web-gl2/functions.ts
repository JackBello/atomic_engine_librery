...
export const loadShader = (context: WebGL2RenderingContext, type: number, source: string) => {
...
export const transformRGB = (color: string): number[] => {
  if (color.match(/rgba([0-9]+, [0-9]+, [0-9], [0-9])/g)) {
    return color.replace("rgb", "").slice(1, -1).split(",").map(value => Number(value.trim()))
  }
...
export const serializeOptions = (context: WebGL2RenderingContext, options: IOptionsNode2D & TOptionsBasic) => {
